...
var robux = {
  ...
  population: 0,
  accounts: 0,
  accountCost: 100000000,
  populationCost: 250000000
}
var settings = {
  autosave: true,
...
function abbreviateButWithDifferentPrefixYesVeryCoolFunctionName(num, fixed, first) {
  if (!settings.abbreviate_numbers) return Math.floor(num);
  if (num === null) { return null; }
  if (num === 0) { return '0'; }
  fixed = (!fixed || fixed < 0) ? 0 : fixed;
  var b = (num).toPrecision(2).split("e"),
    ...
    c = k < 1 ? num.toFixed(0 + fixed) : (num / Math.pow(10, k * 3)).toFixed(1 + fixed),
    d = c < 0 ? c : Math.abs(c),
    e = d + [first ? 'c' : '', 'K', 'M', 'G', 'T', 'P', 'E', 'Z', 'Y'][k];
  return e;
}

function achUpdate() {
  document.getElementById('achCount').innerHTML = `${achievements.unlocked.length}/18 achievements unlocked (${(achievements.unlocked.length / 18 * 100).toFixed(2)}%)`
  if (achievements.unlocked.length >= 18) {
    document.getElementById('achCount').className = 'rainbow'
  } else {
    document.getElementById('achCount').className = ''
  }
  achievements.unlocked.forEach(elem => {
    if (document.getElementById(elem) && typeof elem == "string") {
      document.getElementById(elem).classList = "ach on idk"
      switch (elem) {
        default: break;
...
function update() {
  ...
  document.getElementById("robuxCount").innerHTML = `${abbreviate(robux.generated, 1)} R$`
  ...
  document.getElementById("powerInfo").innerHTML = `${abbreviateButWithDifferentPrefixYesVeryCoolFunctionName(robert.power_left, 1)}W (resets in ${robert.power_reset} second(s))`;
  ...
  document.getElementById("buyAccountBtn").innerHTML = `Buy account<br>(${robux.accounts}, ${abbreviate(robux.accountCost, 1, true)})`
  if (announcer.coins >= robux.accountCost) document.getElementById("buyAccountBtn").classList = "btnLarge on"; else document.getElementById("buyAccountBtn").classList = "btnLarge off";
  if (robux.population < 50) document.getElementById("worldUpgradeBtn").innerHTML = `Increase population multiplier<br>(${(100 + robux.population) / 100}x, ${abbreviate(robux.populationCost, 1, true)})`;
  else document.getElementById("worldUpgradeBtn").innerHTML = `Increase population multiplier<br>(${(100 + robux.population) / 100}x)`;
  if (announcer.coins >= robux.populationCost && robux.population < 50) document.getElementById("worldUpgradeBtn").classList = "btnLarge on"; else document.getElementById("worldUpgradeBtn").classList = "btnLarge off";
  if (robux.generated > 0) document.getElementById("convertRobuxBtn").classList = "btnLarge on"; else document.getElementById("convertRobuxBtn").classList = "btnLarge off";
  if (announcer.level >= 50 && currentTab != 1) { document.getElementById(tabButtons[1]).className = "btn default"; document.getElementById(tabButtons[1]).style = ''; document.getElementById(tabButtons[1]).innerHTML = "Marcus Arena"; }
  if (announcer.level >= 100 && currentTab != 2) { document.getElementById(tabButtons[2]).className = "btn default"; document.getElementById(tabButtons[2]).style = ''; document.getElementById(tabButtons[2]).innerHTML = "Robertium"; document.getElementById("robertiumCount").style = "" }
  if (announcer.level >= 150 && currentTab != 3) { document.getElementById(tabButtons[3]).className = "btn default"; document.getElementById(tabButtons[3]).style = ''; document.getElementById(tabButtons[3]).innerHTML = "Robux"; document.getElementById("robuxCount").style = "" }
  if (announcer.level >= 200 && currentTab != 4) { document.getElementById(tabButtons[4]).className = "btn default"; document.getElementById(tabButtons[4]).style = ''; document.getElementById(tabButtons[4]).innerHTML = "..."; }
  achCheck()
  achUpdate()
...
function tabSet(tab) {
  tabButtons.forEach((element) => document.getElementById(element).className = "btn default")
  tabs.forEach((elementt) => document.getElementById(elementt).style = "display: none")
  document.getElementById(tabButtons[tab]).className = "btn on"
  document.getElementById(tabs[tab]).style = "display: contents";
}
...
function save(notify) {
  ...
  try {
    ...
    localStorage.setItem('lastSave', Math.floor(Date.now() / 1000))
  } catch (err) {
    console.warn(err)
...
function importData() {
  ...
  if (b64.length != 5) {
    ...
    try {
      ...
      update()
    } catch (err) { alertify.notify("Unable to parse data", "error", 4); console.log(err); return } alertify.notify("Imported successfully!", "success", "3"); save()
  }
}
...
function buyAccount() {
  if (announcer.coins < robux.accountCost) {
    alertify.notify("You don't have enough coins for this!", "error", 4); return
  } else {
    announcer.coins -= robux.accountCost
    robux.accountCost = Math.floor(robux.accountCost * 1.08)
    robux.accounts++
    update()
  }
}
...
function upgradeWorld() {
  if (announcer.coins < robux.populationCost) {
    alertify.notify("You don't have enough coins for this!", "error", 4); return
  } else if (robux.population >= 50) {
    alertify.notify("Max level reached!", "error", 4); return
  } else {
    announcer.coins -= robux.populationCost
    robux.populationCost = Math.floor(robux.populationCost * 1.1)
    robux.population++
    update()
  }
}
...
function convertRobux() {
  if (robux.generated <= 0) {
    alertify.notify("You don't have any robux!", "error", 4); return
  } else {
    if (getRandom(1, 150) <= 1) {
      alertify.notify("<b>You got banned!</b><br>All your accounts and Robux were erased. How unfortunate!", "error", 8);
      achGrant("achievement_robuxban")
      robux.accounts = 0
      robux.accountCost = 100000000
      robux.generated = 0
    } else {
      let had = robux.generated
      let got = Math.floor(robux.generated / 80)
      announcer.coins += got
      robux.generated = 0
      alertify.notify(`Sold ${abbreviate(had, 1)} R$ for ${abbreviate(got, 1)} coins!`, "success", 4);
    }
  }
}
...
function robuxCycle() {
  for (i = 1; i <= robux.accounts; i++) {
    if (getRandom(1, 100) <= 20 + robux.population) {
      robux.generated += getRandom(50000, 3000000)
    }
  }
  setTimeout(robuxCycle, 2000)
}
